...
##########################
# HELPERS FOR EXTRACTION FROM RAW DICOM AND XML FILES
##########################
...
def get_patient_table(raw_path, patient_id, k):
    """
    ...
    # table with all slices
    df_all = df1.join(df2, how='left')
    df_all = df_all.reset_index()

    # table with only slices w/ rois (i.e tumors)
    df_rois = df_all.dropna()

    # select max 4 positive and 4 negative images
    k = min(len(df_rois), k)

    # randomly select "k" no-tumor images and "k" tumor images
    idxs = list(random.sample(list(df_rois.index), k))
    for i in range(k):
        while True:
            idx = random.choice(df_all.index)
            if idx not in idxs:
                idxs.append(idx)
                break
    return df_all[df_all.index.isin(idxs)]


##########################
# HELPERS FOR IMAGE PREPROCESSING
##########################
